...
export const mostAnsweredSelector = (state: ReduxState, startDate: Moment, endDate: Moment) => {
  ...
  const moodsInDateRange = filteredDates.reduce((acc: any[], filteredDate) => {
  ...
  const bodyInDateRange = filteredDates.reduce((acc: any[], filteredDate) => {
    const card = answers.cards?.[filteredDate]
    if (!card) return acc
  ...
  const activityInDateRange = filteredDates.reduce((acc: any[], filteredDate) => {
    const card = answers.cards?.[filteredDate]
    if (!card) return acc
  ...
  const flowInDateRange = filteredDates.reduce((acc: any[], filteredDate) => {
    const card = answers.cards?.[filteredDate]
    if (!card) return acc
