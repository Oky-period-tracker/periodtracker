...
class DataController {
    ...
    generateContentTs(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const live = ((_a = request.query) === null || _a === void 0 ? void 0 : _a.live) === 'true';
            // ========== Encyclopedia ========== //
            const encyclopediaRaw = yield this.articleRepository.query(`SELECT ar.id, ca.title as category_title, 
      ...
      `, [request.user.lang]);
            const { articles, categories, subCategories } = (0, core_1.fromEncyclopedia)({
                encyclopediaResponse: encyclopediaRaw,
            });
            // ========== Quiz ========== //
            const quizzesRaw = yield this.quizRepository.find({
                where: { lang: request.user.lang, live },
                order: {
                    topic: 'ASC',
                },
            });
            const { quizzes } = (0, core_1.fromQuizzes)(quizzesRaw);
            // ========== Did you know ========== //
            const didYouKnowsRaw = yield this.didYouKnowRepository.find({
                where: { lang: request.user.lang, live },
            });
            const { didYouKnows } = (0, core_1.fromDidYouKnows)(didYouKnowsRaw);
            // ========== Help ========== //
            const helpCentersRaw = yield this.helpCenterRepository.find({
                where: { lang: request.user.lang },
            });
            const { helpCenters } = (0, core_1.fromHelpCenters)(helpCentersRaw);
            // ========== Avatars ========== //
            const avatarMessagesRaw = yield this.avatarMessagesRepository.find({
                where: { lang: request.user.lang, live },
            });
            const { avatarMessages } = (0, core_1.fromAvatarMessages)(avatarMessagesRaw);
            // ========== Privacy ========== //
            const allPoliciesVersions = yield this.privacyPolicyRepository.find({
                where: { lang: request.user.lang },
            });
            ...
            const latestTermsAndConditions = allTermsAndConditionVersions[allTermsAndConditionVersions.length - 1];
    ...
    generateContentSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const shouldFilter = ((_a = request.query) === null || _a === void 0 ? void 0 : _a.filter) === 'new';
            const encyclopediaRaw = (yield this.articleRepository.query(`SELECT 
      ar.id, 
      ca.id as cat_id, 
      sc.id as subcat_id, 
      ...
      `, [request.user.lang]));
            ...
            const encyclopediaFiltered = encyclopediaRawExtraCols.filter((item) => {
                if (shouldFilter) {
                    const existsInTs = core_1.content[request.user.lang].articles.allIds.includes(item.id);
                    return !existsInTs;
                }
                return true;
            });
            const encyclopediaByCategory = encyclopediaFiltered.reduce((acc, row) => {
                const title = row.category_title_original.length > 31
            ...
            const quizzesFiltered = quizzesRawExtraCols.filter((item) => {
                if (shouldFilter) {
                    const existsInTs = core_1.content[request.user.lang].quizzes.allIds.includes(item.id);
                    return !existsInTs;
                }
                return true;
            });
            ...
            const didYouKnowsFiltered = didYouKnowsRawExtraCols.filter((item) => {
                if (shouldFilter) {
                    const existsInTs = core_1.content[request.user.lang].didYouKnows.allIds.includes(item.id);
                    return !existsInTs;
                }
                return true;
            });
            ...
            const avatarMessagesFiltered = avatarMessagesExtraCols.filter((item) => {
                if (shouldFilter) {
                    const existsInTs = core_1.content[request.user.lang].avatarMessages
            ...
            const encyclopediaSheets = Object.entries(encyclopediaByCategory).reduce((acc, [key, value]) => {
                const worksheet = xlsx_1.default.utils.json_to_sheet(value);
            ...
            const allSheets = Object.assign(Object.assign(Object.assign(Object.assign({}, encyclopediaSheets), otherSheetsWithIds), otherSheets), { Help: helpCentersRaw });
    ...
    uploadContentSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            ...
            const locale = ((_a = request.body.locale) === null || _a === void 0 ? void 0 : _a.length) ? request.body.locale : request.user.lang;
            // Create a workbook from the xlsx data
            const workbook = xlsx_1.default.read(request.file.buffer, { type: 'buffer' });
            ...
            const encyclopediaSheetNames = workbook.SheetNames.filter((name) => !otherSheetNames.includes(name));
            const encyclopediaJson = encyclopediaSheetNames.reduce((acc, name) => {
                const worksheet = workbook.Sheets[name];
                if (!worksheet) {
                    return acc;
                }
                const sheetJson = xlsx_1.default.utils.sheet_to_json(worksheet);
            ...
            const otherJson = otherSheetNames.reduce((acc, name) => {
                ...
                return Object.assign(Object.assign({}, acc), { [name]: json });
            }, {});
            // Generate new Ids for everything if it is a new locale
            const isNewLocale = !core_1.content[locale];
            const { articles, categories, subCategories } = formatEncyclopediaData(encyclopediaJson, isNewLocale);
            const quizzesJson = replaceIdsInJson(otherJson['Quizzes'], otherJson['Quizzes'].map((item) => item.id), isNewLocale);
            const { quizzes } = (0, core_1.fromQuizzes)(quizzesJson);
            const didYouKnowsJson = replaceIdsInJson(otherJson['Did you know'], otherJson['Did you know'].map((item) => item.id), isNewLocale);
            const { didYouKnows } = (0, core_1.fromDidYouKnows)(didYouKnowsJson);
            const avatarMessagesJson = replaceIdsInJson(otherJson['Avatars'], otherJson['Avatars'].map((item) => item.id), isNewLocale);
            const { avatarMessages } = (0, core_1.fromAvatarMessages)(avatarMessagesJson);
            // ========== File ========== //
            const fileContent = `
      // THIS FILE IS AUTO GENERATED. DO NOT EDIT MANUALLY
      import { StaticContent } from '../../../types'
    ...
    generateAppTranslationsSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const buffer = generateSingleTabSheet(core_1.appTranslations[core_1.defaultLocale], 'app');
    ...
    generateCmsTranslationsSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const buffer = generateSingleTabSheet(options_1.cmsTranslations[core_1.defaultLocale], 'cms');
    ...
    uploadAppTranslationsSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            // Ensure a file was uploaded
            if (!request.file || !request.file.buffer) {
    ...
    export const ${locale}: AppTranslations = ${JSON.stringify(Object.assign({ empty: '' }, sheetData))}
    `;
            const fileName = `${locale}.ts`;
            // Set the headers to inform the browser about file type and suggested filename
            response.setHeader('Content-disposition', 'attachment; filename=' + fileName);
    ...
    uploadCmsTranslationsSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            // Ensure a file was uploaded
            if (!request.file || !request.file.buffer) {
    ...
    generateCountriesSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            // Create a new workbook
            const workbook = {
                SheetNames: [],
            ...
            const headers = ['code', ...Object.keys(Object.values(core_1.countries)[0])];
            const dataArray = Object.entries(core_1.countries).map(([key, value]) => [
                key,
                ...Object.values(value),
            ]);
            const sheetArray = [headers, ...dataArray];
            // Convert the array of arrays to a worksheet
            const worksheet = xlsx_1.default.utils.aoa_to_sheet(sheetArray);
    ...
    generateProvincesSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const buffer = generateSingleTabSheetFromArray(core_1.provinces, 'provinces');
    ...
    uploadCountriesSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            // Ensure a file was uploaded
            if (!request.file || !request.file.buffer) {
                return response.status(400).send('No file was uploaded.');
            ...
            const jsonRaw = xlsx_1.default.utils.sheet_to_json(worksheet, {
                header: 1,
            });
            ...
            const result = reformattedArray.reduce((acc, item) => {
                // @ts-ignore
                const { code } = item, rest = __rest(item, ["code"]);
                acc[code] = rest;
                return acc;
            }, {});
            const fileContent = `
    ...
    uploadProvincesSheet(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            // Ensure a file was uploaded
            if (!request.file || !request.file.buffer) {
                return response.status(400).send('No file was uploaded.');
...
const getSimpleSheetData = (buffer, flatten = true) => {
    ...
    if (flatten) {
        return Object.entries(output).reduce((acc, [key, item]) => {
            if (typeof item === 'object') {
                return Object.assign(Object.assign({}, acc), item);
            }
            return Object.assign(Object.assign({}, acc), { [key]: item });
        }, {});
    }
    return output;
};
...
const replaceIdsInJson = (data, idsToReplace, shouldReplace) => {
    if (!shouldReplace) {
        return data;
    }
    const dataString = JSON.stringify(data);
    const dataNewIds = idsToReplace.reduce((acc, id) => {
        const uuidRegex = new RegExp(id, 'g');
        return acc.replace(uuidRegex, (0, uuid_1.v4)());
    }, dataString);
...
const formatEncyclopediaData = (encyclopediaJson, shouldReplace) => {
    ...
    const data = (0, core_1.fromEncyclopedia)({ encyclopediaResponse: dataNoOriginals });
    if (!shouldReplace) {
        return data;
    }
