...
export const AvatarPreview: React.FC<AvatarPreviewProps> = ({
  ...
  const bodySize: 'small' | 'medium' | 'large' = React.useMemo(() => {
    if (bodyType === 'small') return 'small'
    if (bodyType === 'large') return 'large'
    return 'medium'
  }, [bodyType])
  ...
  const BodySvg = getAvatarAsset('body', bodyType, bodySize)
  const HairSvg = getAvatarAsset('hair', hairStyle, bodySize)
  const SmileSvg = smile ? getAvatarAsset('smile', smile, bodySize) : null
  const EyeSvg = getAvatarAsset('eyes', eyeShape, bodySize)
  const ClothingSvg = getAvatarAsset('clothing', clothing, bodySize)
  const DeviceSvg = getAvatarAsset('devices', devices, bodySize)
