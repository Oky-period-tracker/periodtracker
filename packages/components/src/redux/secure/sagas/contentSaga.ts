...
function* onRehydrate(action: RehydrateAction) {
  const locale = yield select(secureSelectors.currentLocaleSelector)

  const hasPreviousContentFromStorage = action.payload && action.payload.content

  if (!hasPreviousContentFromStorage) {
    yield put(secureActions.initStaleContent(staleContent[locale]))
  }
  ...
  if (shouldFetch) {
    yield put(secureActions.fetchContentRequest(locale))
  }
}
...
function* onFetchSurveyContent(
  ...
  const locale = yield select(secureSelectors.currentLocaleSelector)
  const userID = yield select(secureSelectors.currentUserSelector)
  try {
    const surveys = yield httpClient.fetchSurveys({
      locale,
      userID,
    })
    const previousSurveys = yield select(secureSelectors.allSurveys)
    const completedSurveys = yield select(secureSelectors.completedSurveys)
    ...
    yield put(secureActions.updateAllSurveyContent(finalArr))
  } catch (error) {
    //
  }
}
...
function* onFetchContentRequest(action: ExtractActionFromActionType<'FETCH_CONTENT_REQUEST'>) {
  ...
  try {
    ...
    yield put(
      secureActions.fetchContentSuccess({
    ...
    yield put(secureActions.fetchContentFailure())
    const aboutContent = yield select(secureSelectors.aboutContent)
    if (!aboutContent) {
      const localeInit = yield select(secureSelectors.currentLocaleSelector)
      yield put(secureActions.initStaleContent(staleContent[localeInit]))
    }
  }
}
