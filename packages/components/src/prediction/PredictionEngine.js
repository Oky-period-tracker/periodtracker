...
class PredictionEngine {
    constructor(state, listener = () => null) {
        this.state = state;
    ...
    // +=+=+=+=+=+=+=+=+=+=+=+=+=+ Main Engine +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
    predictDay(inputDay, isVerified = false, selectedDayInfo = {}) {
        let onPeriod = false;
        let onFertile = false;
        ...
        if (!this.state.isActive) {
            ...
            return {
                onPeriod,
                onFertile,
                date: (0, moment_1.default)(inputDay).startOf('day'),
                cycleDay: 0,
                daysLeftOnPeriod: 0,
                cycleStart: (0, moment_1.default)().startOf('day'),
        ...
        if (diffDays >= cycleLength && diffDays > 0) {
            ...
            const numberOfCyclesAheadOfCurrent = Math.floor((diffDays - cycleLength) / this.state.smartPrediction.smaCycleLength);
            cycleStart = (0, moment_1.default)(cycleStart)
                .clone()
                .add(cycleLength + numberOfCyclesAheadOfCurrent * this.state.smartPrediction.smaCycleLength, 'days');
            cycleDay =
                diffDays -
                    cycleLength -
        ...
        if (diffDays < 0) {
            ...
            if (lodash_1.default.isEmpty(relevantCycleHistoryEntry)) {
                const log = {
                    onPeriod,
                    onFertile,
                    date: (0, moment_1.default)(inputDay).startOf('day'),
                    cycleDay: 0,
                    daysLeftOnPeriod: 0,
                    cycleStart: (0, moment_1.default)(0),
                    daysUntilNextPeriod: 0,
                    cycleLength: 100,
                    periodLength: 0,
                };
    ...
    // +=+=+=+=+=+=+=+=+=+ Range Population +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
    calculateStatusForDateRange(startDate, endDate, verifiedPeriodsData, hasFuturePredictionActive) {
        var _a;
        const loop = (0, moment_1.default)(startDate);
        let markedDates = {};
        while (loop <= endDate) {
            const newDate = (0, moment_1.default)(loop.date(loop.date() + 1)).startOf('day');
            ...
            if (onPeriod || onFertile) {
                ...
                tempArr.forEach((element, index) => {
                    if ((0, moment_1.default)(element).isSame((0, moment_1.default)(date).format('YYYY-MM-DD'))) {
                        let periodDayValue = {};
                        periodDayValue = tempValuesArr[index];
                        if (Object.keys(periodDayValue).includes('periodDay')) {
                            if (periodDayValue === null || periodDayValue === void 0 ? void 0 : periodDayValue.periodDay) {
                                dateExists = true;
                            }
                        }
                    }
                ...
                if (!hasFuturePredictionActive) {
                    if (!lodash_1.default.isEmpty(this.state.actualCurrentStartDate)) {
                        const checkStartDate = ((_a = this.state.actualCurrentStartDate) === null || _a === void 0 ? void 0 : _a.cycleStart)
                            ? this.state.actualCurrentStartDate.cycleStart
                            : this.state.actualCurrentStartDate.startDate;
                        isFutureDate = (0, moment_1.default)(newEntry).isAfter((0, moment_1.default)(checkStartDate).add(this.state.actualCurrentStartDate.periodLength, 'days'));
                    }
                    if (isFutureDate) {
                        color = '#91d9e2';
                ...
                markedDates = Object.assign(Object.assign({}, markedDates), { [newEntry]: {
                        customStyles: {
                            container: {
                                borderColor,
                                borderWidth: 2,
                            ...
                            text: {
                                color: !hasFuturePredictionActive && isFutureDate
                                    ? 'white'
                                    : onPeriod
                                        ? dateExists
                                            ? 'white'
    ...
    calculateFullInfoForDateRange(startDate, endDate) {
        const loop = (0, moment_1.default)(startDate).startOf('day');
        const dateArray = [];
        while (loop <= endDate) {
            const newDate = (0, moment_1.default)(loop.date(loop.date() + 1)).startOf('day');
    ...
    // +=+=+=+=+==+=+=+ User Input Handling +=+=+=+=+=+=+=+=++=+=+=+=+=+=+=+==+=+=+=+=+
    userInputDispatch({ type, inputDay, errorCallBack = (err) => null, getPredictedCycles = (flag) => null, }) {
    ...
    _currentDayChecking() {
        ...
        const tempCurrentCycle = Object.assign({}, this.state.actualCurrentStartDate);
        const todayDate = (0, moment_1.default)().startOf('day');
        ...
        if (diffDays >= this.state.currentCycle.cycleLength) {
            ...
            const numberOfCyclesAheadOfCurrent = Math.ceil(daysAheadOfCurrentCycleEnd / this.state.smartPrediction.smaCycleLength);
            ...
            while (i < numberOfCyclesAheadOfCurrent) {
                ...
                const dayToCheck = startingPoint
                    .clone()
                    .add(this.state.currentCycle.cycleLength + i * this.state.smartPrediction.smaCycleLength, 'days');
                if ((!this.state.futurePredictionStatus &&
                    (0, moment_1.default)(dayToCheck).isSameOrBefore((0, moment_1.default)(tempCurrentCycle.cycleStart), 'day')) ||
                    ...
                    this.state.currentCycle = {
                        startDate: (0, moment_1.default)(tempCurrentCycle.cycleStart),
                        periodLength: tempCurrentCycle.periodLength,
                        cycleLength: (0, moment_1.default)().diff(tempCurrentCycle.cycleStart, 'days') + 7,
                    };
                }
                i += 1;
            }
    ...
    _adjustMenstruatingHandler(inputDay, errorCallBack, getPredictedCycles) {
        ...
        if (daysFromStart < this.state.currentCycle.periodLength / 2) {
            const tempDate = (0, moment_1.default)(inputDay).add(1, 'days');
            if (daysFromStart <= 0) {
                const daysFromCurrentCycle = this.state.currentCycle.startDate.diff(tempDate, 'days');
                const remainingPeriodLenghth = this.state.currentCycle.periodLength + daysFromCurrentCycle - 1;
                if (tempDate.clone().add(1, 'days').isAfter((0, moment_1.default)()) && remainingPeriodLenghth > 0) {
                ...
                if ((0, moment_1.default)(inputDay).isSame((0, moment_1.default)().format('YYYY-MM-DD')) && !this.state.history.length) {
        ...
        if (daysFromStart - this.state.currentCycle.periodLength < 2) {
            ...
            const updateCurrentCycle = Object.assign(Object.assign({}, this.state.currentCycle), { periodLength: daysFromStart + 1 });
            this.state.currentCycle = updateCurrentCycle;
            this.state.actualCurrentStartDate = updateCurrentCycle;
            getPredictedCycles(true);
            return;
    ...
    _startedEarlyHandler(inputDay, errorCallBack, getPredictedCycles) {
        const daysFromStart = inputDay.diff(this.state.currentCycle.startDate, 'days');
    ...
    _addNewCycleInHistory(inputDay, errorCallBack, getPredictedCycles) {
        ...
        const tempDate = (0, moment_1.default)(this.state.currentCycle.startDate).add(1, 'days');
        if (lodash_1.default.isEmpty(relevantCycleHistoryEntry.closetCycle)) {
            ...
            if (tempHistoryArr.length) {
                if (inputDay.diff(tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate, 'days') >= -4) {
                    const updateLastHistoryCycle = Object.assign(Object.assign({}, tempHistoryArr[tempHistoryArr.length - 1]), { cycleStartDate: inputDay, cycleLength: tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate.diff(inputDay, 'days') +
                            tempHistoryArr[tempHistoryArr.length - 1].cycleLength, periodLength: tempHistoryArr[tempHistoryArr.length - 1].periodLength +
                ...
                const addNewHisCycle = {
                    ...
                    cycleEndDate: tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate
                        .clone()
                        .subtract(1, 'days'),
                    cycleLength: tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate.diff(inputDay, 'days'),
                    periodLength: 1,
                };
            ...
            if (lodash_1.default.isEmpty(this.state.currentCycle)) {
            ...
            if ((0, moment_1.default)(this.state.currentCycle.startDate).isSame((0, moment_1.default)().format('YYYY-MM-DD'))) {
        ...
        const daysFromInputToCloset = inputDay.diff(this.state.history[relevantCycleHistoryEntry.closestIndex].cycleStartDate, 'days');
        // MARK:-- If selected date is between last history cycle and current cycle
        if ((0, moment_1.default)(inputDay).isAfter(this.state.history[0].cycleStartDate)) {
            ...
            if (daysFromCurrentCycle >= -4 &&
                ...
                const updateCurrentCycle = Object.assign(Object.assign({}, this.state.currentCycle), { startDate: inputDay, cycleLength: this.state.currentCycle.cycleLength + -daysFromCurrentCycle, periodLength: this.state.currentCycle.periodLength + -daysFromCurrentCycle });
            ...
            if (daysFromInputToCloset - this.state.history[0].periodLength < 2) {
                ...
                if (inputDay.diff(this.state.currentCycle.startDate, 'days') >= 0) {
                    ...
                    const updateCurrentCycle = Object.assign(Object.assign({}, this.state.currentCycle), { startDate: this.state.history[0].cycleStartDate, cycleLength: this.state.currentCycle.cycleLength + this.state.history[0].cycleLength, periodLength: this.state.currentCycle.periodLength +
        ...
        const periodLengthOfClosestCycle = this.state.history[relevantCycleHistoryEntry.closestIndex]
            .periodLength;
        // check conditions to add new cylce between 2 history cyles
        const daysFromNextCycle = inputDay.diff(this.state.history[relevantCycleHistoryEntry.closestIndex - 1].cycleStartDate, 'days');
        ...
        if (daysFromInputToCloset > 0) {
            ...
            if (daysFromNextCycle >= -4 && daysFromInputToCloset - periodLengthOfClosestCycle >= 2) {
                ...
                const updatePreviousCycle = Object.assign(Object.assign({}, relevantCycleHistoryEntry.closetCycle), { cycleEndDate: inputDay.clone().subtract(1, 'days'), cycleLength: inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days') });
            ...
            if (daysFromInputToCloset - periodLengthOfClosestCycle >= 2) {
                ...
                const newHistoryEntry = {
                    ...
                    cycleLength: Math.abs(inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleEndDate, 'days')),
                    periodLength: 1,
                };
                const updateClosestHistoryCycle = {
                    cycleStartDate: relevantCycleHistoryEntry.closetCycle.cycleStartDate,
                    cycleEndDate: inputDay.clone().subtract(1, 'days'),
                    cycleLength: Math.abs(inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days')),
                    periodLength: relevantCycleHistoryEntry.closetCycle.periodLength,
                };
                let deletedTempArr = [];
                deletedTempArr = tempHistoryArr.splice(relevantCycleHistoryEntry.closestIndex, this.state.history.length - 1, newHistoryEntry);
            ...
            if (daysFromInputToCloset - periodLengthOfClosestCycle < 2) {
                ...
                if (inputDay.diff(this.state.history[relevantCycleHistoryEntry.closestIndex - 1].cycleStartDate, 'days') <= -2) {
                    const updateClosestHistoryCycle = {
                        cycleStartDate: relevantCycleHistoryEntry.closetCycle.cycleStartDate,
                        cycleEndDate: relevantCycleHistoryEntry.closetCycle.cycleEndDate,
                        cycleLength: relevantCycleHistoryEntry.closetCycle.cycleLength,
                        periodLength: inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days') + 1,
                    };
                ...
                }
                {
                    const updateClosestHistoryCycle = {
                        ...
                        periodLength: inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days') +
                            1 +
    ...
    _adjustCurrentStartDateHandler(inputDay, errorCallBack, getPredictedCycles) {
        const daysFromStart = inputDay.diff(this.state.currentCycle.startDate, 'days');
        ...
        if (daysFromStart < 0 && daysFromPreviousStart > 0) {
            const relevantCycleHistoryEntry = this._getClosetCycleHistoryInfo(inputDay);
            if (inputDay.diff(this.state.history[0].cycleStartDate, 'days') -
                ...
                const updateHistory = Object.assign(Object.assign({}, relevantCycleHistoryEntry.closetCycle), { periodLength: inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days') + 1 });
        ...
        // --------- if between current cycle start and next cycle start ----------
            ...
            this.state.currentCycle.cycleLength += -daysFromStart;
            this.state.currentCycle.periodLength += -daysFromStart;
            const updateHistory = Object.assign(Object.assign({}, this.state.history[0]), { cycleEndDate: inputDay.clone().subtract(1, 'days'), cycleLength: inputDay.diff(this.state.history[0].cycleStartDate, 'days') });
            this.state.history[0] = updateHistory;
        }
        // Check If the current cycle status must change from the above actions
        const todayInfo = this.predictDay((0, moment_1.default)().startOf('day'));
    ...
    _adjustFutureStartDateHandler(inputDay, errorCallBack, getPredictedCycles) {
        const daysFromStart = inputDay.diff(this.state.currentCycle.startDate, 'days');
        if (daysFromStart < 50 && daysFromStart > 10) {
            this.state.currentCycle.cycleLength = daysFromStart;
        }
    }
    _adjustFutureMenstruatingHandler(inputDay, errorCallBack, getPredictedCycles) {
        errorCallBack('too_far_ahead');
    }
    _getClosetCycleHistoryInfo(inputDay) {
    ...
    _cycleCompletion(completionDate, getPredictedCycles) {
        return __awaiter(this, void 0, void 0, function* () {
            const adjustedCycleLength = completionDate.diff(this.state.currentCycle.startDate, 'days');
            this.state.history.unshift({
                cycleStartDate: this.state.currentCycle.startDate,
                cycleEndDate: (0, moment_1.default)(completionDate.clone().subtract(1, 'days')).startOf('day'),
                periodLength: this.state.currentCycle.periodLength,
                cycleLength: adjustedCycleLength,
            });
            this.state.currentCycle.startDate = (0, moment_1.default)(completionDate.valueOf()).startOf('day');
            this.state.smartPrediction.circularPeriodLength.push(this.state.currentCycle.periodLength);
            this.state.smartPrediction.smaPeriodLength = Math.round(arrayAverage(this.state.smartPrediction.circularPeriodLength.toarray()));
            ...
            this.state.smartPrediction.smaCycleLength = Math.round(arrayAverage(this.state.smartPrediction.circularCycleLength.toarray()));
            this.state.currentCycle.cycleLength = this.state.smartPrediction.smaCycleLength;
            // TODO: this is sometimes null ?
            getPredictedCycles === null || getPredictedCycles === void 0 ? void 0 : getPredictedCycles(true);
        });
    }
    // +=+=+=+=+==+=+=+=+= History Adjust Methods +=+=+=+=+=+=+=+=++=+=+=+=+=+=+=+==+=+=+=+=+
    _adjustHistoryStartDateHandler(inputDay, errorCallBack, getPredictedCycles) {
        ...
        if (lodash_1.default.isEmpty(closetCycle)) {
            const daysFromStart = inputDay.diff(this.state.history[this.state.history.length - 1].cycleStartDate, 'days');
            const periodLengthToBe = this.state.history[this.state.history.length - 1].periodLength + -daysFromStart;
            const cycleLengthToBe = this.state.history[this.state.history.length - 1].cycleLength + -daysFromStart;
        ...
        if (cycleDay >= Math.round(cycleLength / 2)) {
            ...
            if (morePresentHistoryCycle && morePresentHistoryCycle.cycleStartDate) {
                if (inputDay.diff(morePresentHistoryCycle.cycleStartDate, 'days') > -5) {
                    if (currentCycleLengthToBe - closetCycle.periodLength >= 7) {
                        const updateHistoryNextCycle = Object.assign(Object.assign({}, morePresentHistoryCycle), { cycleStartDate: inputDay, cycleLength: morePresentHistoryCycle.cycleLength += -daysFromStart, periodLength: morePresentHistoryCycle.periodLength += -daysFromStart });
            ...
            const updateHistoryThisCycle = Object.assign(Object.assign({}, closetCycle), { cycleEndDate: inputDay.clone().subtract(1, 'days'), cycleLength: currentCycleLengthToBe });
        ...
        if (cycleDay < closetCycle.periodLength / 2) {
            ...
            if (this.state.history[closestIndex].periodLength + Math.abs(daysFromRelevantStart) <
                minPeriodLength) {
            ...
            if (morePastHistoryCycle) {
                ...
                this.state.history[indexMorePast].cycleLength = inputDay.diff(this.state.history[indexMorePast].cycleStartDate, 'days');
            }
        }
    }
    _adjustMenstruatingHistoryHandler(inputDay, errorCallBack, getPredictedCycles) {
        const daysFromStart = inputDay.diff(this.state.currentCycle.startDate, 'days');
        const relevantCycleHistoryEntry = this._getClosetCycleHistoryInfo(inputDay);
        ...
        if (relevantCycleHistoryEntry.closestIndex - 1 >= 0) {
            ...
            const tempDate = (0, moment_1.default)(inputDay).add(1, 'days');
            if (tempDate.isSame(this.state.history[relevantCycleHistoryEntry.closestIndex - 1].cycleStartDate) &&
                ...
                const updateCurrentHistoryCycle = {
                    ...
                    cycleLength: this.state.history[relevantCycleHistoryEntry.closestIndex - 1].cycleEndDate.diff(inputDay, 'days'),
                    periodLength: this.state.history[relevantCycleHistoryEntry.closestIndex - 1].periodLength -
                        this.state.history[relevantCycleHistoryEntry.closestIndex - 1].cycleStartDate.diff(inputDay, 'days'),
                };
                const updateClosestHistoryCycle = {
        ...
        if (daysFromStart < 0 && !lodash_1.default.isEmpty(relevantCycleHistoryEntry.closetCycle)) {
            const lengthToBe = inputDay.diff(relevantCycleHistoryEntry.closetCycle.cycleStartDate, 'days') + 1;
            if (lengthToBe < 1) {
                // errorCallBack('period_too_short')
                return;
            }
