...
databaseTests('survey transactions with PostgreSQL', () => {
  ...
  beforeAll(async () => {
    mockConnection = await createConnection({
      name: schema,
      type: 'postgres',
      host: process.env.CMS_TEST_DATABASE_SOCKET,
      port: Number(process.env.CMS_TEST_DATABASE_PORT),
  ...
  async function create() {
    return controller.save(
      req({ live: false, questions: [question, { ...question, sort_number: '2' }] }),
      response,
      jest.fn(),
    )
  }
  async function failParent(operation: 'UPDATE' | 'DELETE', action: () => Promise<any>) {
    await mockConnection.query(
      `CREATE FUNCTION ${schema}.reject_write() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'forced parent failure'; END; $$`,
    )
    await mockConnection.query(
      `CREATE TRIGGER reject_write BEFORE ${operation} ON ${schema}.survey FOR EACH ROW EXECUTE PROCEDURE ${schema}.reject_write()`,
    )
    try {
      await expect(action()).rejects.toThrow('forced parent failure')
    } finally {
  ...
  it('rolls back the parent and earlier children when one question fails', async () => {
    await expect(
      controller.save(
        req({ live: false, questions: [question, { ...question, question: null }] }),
        response,
        jest.fn(),
      ),
    ).rejects.toThrow()
  ...
  it('rolls back question edits and deletions when updating the parent fails', async () => {
    ...
    await failParent('UPDATE', () =>
      controller.update(
        req(
          {
            questions: [{ ...children[0], question: 'Changed' }],
            deletedQuestion: [children[1].id],
            live: 'true',
          },
          saved!.id,
        ),
    ...
    expect((await mockConnection.getRepository(Question).findOne(children[0].id))!.question).toBe(
      'Original',
    )
  ...
  it('rejects a question from another survey and rolls back earlier edits', async () => {
    ...
    await expect(
      controller.update(
        req(
          {
            questions: [
              { ...mine, question: 'Changed' },
              { ...theirs, question: 'Hijacked' },
            ],
          },
          first!.id,
        ),
    ...
    expect((await mockConnection.getRepository(Question).findOne(mine.id))!.question).toBe(
      'Original',
    )
    expect((await mockConnection.getRepository(Question).findOne(theirs.id))!.question).toBe(
      'Original',
    )
  })
})
