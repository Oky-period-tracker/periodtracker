...
const clothingDir = path.join(__dirname, '../src/resources/assets/images/avatars/friend/avatar-parts/clothing');
const svgFiles = fs.readdirSync(clothingDir).filter(file => file.endsWith('.svg')).sort();
...
function getComponentName(filePath) {
  ...
  return 'Clothing' + pascalParts.join('');
}
...
function convertSvgToJsx(svgPath, componentName) {
  ...
  // Split by lines to process them one by one
...
for (const svgFile of svgFiles) {
  const svgPath = path.join(clothingDir, svgFile);
...
`;
  ...
  animatedPropsCode.push(`  // Animated props for root SVG - applies jump to entire clothing
  ...
  if (hasLeftHand) {
    animatedPropsCode.push(`  // Animated props for left hand - rotate around shoulder joint (75, 94)
  // Only apply transform if rotation is non-zero to avoid affecting non-animated avatars
  const leftHandAnimatedProps = useAnimatedProps(() => {
    ...
    const shoulderX = 75
    const shoulderY = 94
    // To rotate around a point: translate to point, rotate, translate back
    return {
      transform: [
  ...
  if (hasRightHand) {
    animatedPropsCode.push(`  // Animated props for right hand - rotate around shoulder joint (99, 94)
  // Only apply transform if rotation is non-zero to avoid affecting non-animated avatars
...
const outputFile = path.join(clothingDir, 'ClothingComponents.tsx');
