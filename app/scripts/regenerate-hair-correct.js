...
const animatedClasses = ['leftHand', 'rightHand', 'leftLeg', 'rightLeg'];

// Function to get component name from file path
function getComponentName(filePath) {
  const fileName = path.basename(filePath, '.svg');
  // Hair files are like "hair01.svg", "hair02.svg", etc.
  const match = fileName.match(/hair(\d+)/i);
  if (match) {
    const num = match[1].padStart(2, '0');
    return `Hair${num}`;
  }
  // Fallback: capitalize first letter
  return 'Hair' + fileName.charAt(0).toUpperCase() + fileName.slice(1);
}
...
function convertSvgToJsx(svgPath, componentName) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (gOpenMatch) {
      ...
      if (animatedClasses.includes(className)) {
        // Convert to AnimatedG with props (only hands and legs, not head/chest)
  ...
  // Replace fill="currentColor" with fill={fillColor} to apply the color prop
...
for (const svgFile of svgFiles) {
  const svgPath = path.join(hairDir, svgFile);
...
`;
  ...
  // Animated props for root SVG - applies jump to entire hair
...
const outputFile = path.join(hairDir, 'HairComponents.tsx');
