-- ============================================================================
-- PeriodTracker Complete Database Setup
-- This script will DROP and recreate the entire database schema
-- !!! WARNING: THIS WILL DELETE ALL EXISTING DATA !!!
-- ============================================================================

-- Drop existing schema (THIS DELETES EVERYTHING!)
-- ============================================================================
DROP SCHEMA IF EXISTS periodtracker CASCADE;
...
CREATE OR REPLACE VIEW "periodtracker"."answered_surveys" AS SELECT (periodtracker.app_event.payload ->> 'id'::text) AS id,
    ...
    (periodtracker.app_event.payload ->> 'user_id'::text) AS user_id,
...
-- Migration 1709716115361: Add sorting keys to category, subcategory, and article
-- ============================================================================
...
-- Migration 1695772800000: Create video table
-- ============================================================================
...
-- Migration 1701781248594: Update oky_user table with signup dates
-- ============================================================================
...
-- Migration 1705982678530: Alter video table (remove parent_category)
-- ============================================================================
ALTER TABLE "periodtracker"."video"
DROP COLUMN "parent_category";


-- Migration 1714105338916: Add voice over to articles
-- ============================================================================
ALTER TABLE "periodtracker".article
ADD "voiceOverKey" text DEFAULT NULL;


-- Migration 1715142512486: Add user metadata
-- ============================================================================
ALTER TABLE periodtracker.oky_user
ADD COLUMN metadata json DEFAULT '{}';


-- Migration 1717481917580: Add video sorting key
-- ============================================================================
...
-- Migration 1724617170978: Help center enhancements
-- ============================================================================
...
-- Migration 1740038351971: Content restriction columns
-- ============================================================================
ALTER TABLE "periodtracker".article
...
-- 
-- NEXT STEPS:
-- 1. Log into CMS at http://localhost:5000 with admin/admin
-- 2. Create a new secure admin user via /user-management
-- 3. Log out and log back in as your new user
-- 4. Delete the temporary 'admin' user for security
-- ============================================================================
