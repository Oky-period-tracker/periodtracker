...
const animatedClasses = ['head'];

// Function to get component name from file path
function getComponentName(filePath) {
  const fileName = path.basename(filePath, '.svg');
  // Smile files are like "smile.svg"
  if (fileName.toLowerCase() === 'smile') {
    return 'SmileSmile';
  }
  // Fallback: capitalize first letter
  return 'Smile' + fileName.charAt(0).toUpperCase() + fileName.slice(1);
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (gOpenMatch) {
      ...
      if (animatedClasses.includes(className)) {
        // Convert to regular G (head is now regular G, not animated)
...
for (const svgFile of svgFiles) {
  const svgPath = path.join(smileDir, svgFile);
...
`;
  ...
  // Animated props for root SVG - applies jump to entire smile
...
const outputFile = path.join(smileDir, 'SmileComponents.tsx');
