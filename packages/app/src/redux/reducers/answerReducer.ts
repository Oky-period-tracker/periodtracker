  ...
  if (action.type === "ANSWER_DAILY_CARD") {
    ...
    if (
      ...
      if (typeof state[keyCard][action.payload.cardName] === "string") {
        ...
        answersToInsert = [state[keyCard][action.payload.cardName]].concat(
          action.payload.answer
        );
      } else {
        if (
          state[keyCard][action.payload.cardName].includes(
            action.payload.answer
          )
        ) {
...
export function answerReducer(
  ...
  if (action.type === "ANSWER_QUIZ") {
    return {
      ...state,
      [action.payload.userID]: answerForUserReducer(
