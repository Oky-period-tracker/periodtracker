...
export class PredictionEngine {
  ...
  private _currentDayChecking() {
    ...
    if (diffDays >= this.state.currentCycle.cycleLength) {
      ...
      let i = 0
      // if((!this.state.futurePredictionStatus && startingPoint.isBefore(tempCurrentCycle)) || this.state.futurePredictionStatus) {
      while (i < numberOfCyclesAheadOfCurrent) {
        ...
        // console.log('********* ------- ',dayToCheck, moment(dayToCheck).isSameOrBefore(moment(tempCurrentCycle.cycleStart), 'day'));
        ...
        if (
          ...
          // console.log('step 1 ------------',dayToCheck, tempCurrentCycle.cycleStart);
  ...
  private _adjustMenstruatingHandler(inputDay, errorCallBack, getPredictedCycles) {
    ...
    // MARK:- If selected day is from the current cycle period days
    // if(moment(inputDay).isSameOrAfter){
    ...
    if (daysFromStart < this.state.currentCycle.periodLength / 2) {
      ...
      if (daysFromStart <= 0) {
        ...
        if (moment(inputDay).isSame(moment().format('YYYY-MM-DD')) && !this.state.history.length) {
          // if(moment(inputDay).isSame(moment()) && !this.state.history.length){
        ...
        // MARK:- If there is cycles in history and today's date is the period day of current cycle, then prevent current cycle going to
        // if(){

        // }
        //   const updateCurrentCycle = {
        //     startDate: tempDate.clone().add(1, 'days'),
        //     cycleLength: this.state.currentCycle.cycleLength + daysFromCurrentCycle - 1,
        //     periodLength: remainingPeriodLenghth,
        //   }
        //   this.state.currentCycle = updateCurrentCycle
        //   this.state.actualCurrentStartDate = updateCurrentCycle
        //   getPredictedCycles(true)
    ...
    // geeta
    // if(!this.state.history.length ){
    //   const tempDate = inputDay.clone().add(1, 'days')
    // debugger
    //   //MARK:- If selected day is the 1st period day
    //   if(inputDay.diff(this.state.currentCycle.startDate, 'days') < 0 ){
    //     if(this.state.currentCycle.periodLength >1){
    //       this.state.currentCycle.startDate = tempDate.clone().add(1, 'days')
    //       this.state.currentCycle.cycleLength += daysFromStart
    //       this.state.currentCycle.periodLength += daysFromStart
    //       getPredictedCycles(true)
    //       return
    //     } else {
    //       this.state.currentCycle.startDate = tempDate
    //       this.state.currentCycle.periodLength = 0
    //       getPredictedCycles(true)
    //       return
    //     }
    //   }
    ...
    //   this.state.currentCycle.periodLength += -daysFromStart
    //   getPredictedCycles(true)
    //   this._adjustCurrentStartDateHandler(tempDate, errorCallBack, getPredictedCycles)
    //   return
    // }
    if (daysFromStart - this.state.currentCycle.periodLength < 2) {
      if (daysFromStart < 0) {
      ...
      const updateCurrentCycle = {
        ...
        // cycleLength: this.state.currentCycle.cycleLength - (this.state.currentCycle.periodLength -daysFromCurrentCycle)
      }
      this.state.currentCycle = updateCurrentCycle
      this.state.actualCurrentStartDate = updateCurrentCycle
      getPredictedCycles(true)
  ...
  private _startedEarlyHandler(inputDay, errorCallBack, getPredictedCycles) {
    ...
    // debugger
    // if (
    //   moment(inputDay).diff(moment(this.state.currentCycle.startDate), 'days') < 2 &&
    //   !inputDay.onPeriod
    // ) {
    //   // console.log('step 1 -------')
    // }
  ...
  private _adjustCurrentStartDateHandler(inputDay, errorCallBack, getPredictedCycles) {
    ...
    if (daysFromStart < 0 && daysFromPreviousStart > 0) {
      ...
      // this.state.history[relevantCycleHistoryEntry.closestIndex] = updateHistory
      //////// -----------8***********************
      /// -------- change from current cycle to cylce at index 0 in history array ----------------\\\
      // this.state.currentCycle.startDate = inputDay
      // this.state.currentCycle.cycleLength += -daysFromStart
      // this.state.currentCycle.periodLength += -daysFromStart
    }
