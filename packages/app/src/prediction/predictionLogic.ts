...
export function decisionProcessPeriod({
  ...
  if (
    (diffFromStart < 0 &&
      selectedDayInfo.cycleDay > selectedDayInfo.cycleLength / 2) ||
...
export function decisionProcessNonPeriod({
  ...
  if (
    (diffFromStart < 0 &&
      selectedDayInfo.cycleDay < selectedDayInfo.periodLength / 2) ||
  ...
  if (
    diffFromStart < 0 &&
    selectedDayInfo.cycleDay < currentCycleInfo.cycleLength / 2
  ) {
    // cant reduce period to less than 14 days into the previous cycle and if current length is more than 10
    return {
