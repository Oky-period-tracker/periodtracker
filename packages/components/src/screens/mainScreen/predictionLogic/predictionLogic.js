...
exports.decisionProcessPeriod = decisionProcessPeriod;
exports.decisionProcessNonPeriod = decisionProcessNonPeriod;
function decisionProcessPeriod({ inputDay, // moment.Moment
...
function decisionProcessNonPeriod({ inputDay, selectedDayInfo, currentCycleInfo, history, isActive, }) {
    const diffFromStart = inputDay.diff(currentCycleInfo.cycleStart, 'days');
    if (!isActive && diffFromStart >= 0) {
        return {
            type: 'adjust-mens-end',
    ...
    if ((diffFromStart < 0 && selectedDayInfo.cycleDay < selectedDayInfo.periodLength / 2) ||
...
//# sourceMappingURL=predictionLogic.js.map