...
export function fromEncyclopedia({
  ...
  encyclopediaResponse.forEach((item) => {
    ...
    // Subcategory
    const subcategoryAlreadyExists =
    ...
    if (!subcategoryAlreadyExists) {
      ...
      dataShape.categories.byId[item.cat_id] = {
        ...dataShape.categories.byId[item.cat_id],
        subCategories: dataShape.categories.byId[
          item.cat_id
        ].subCategories.concat(item.subcat_id),
      };
    }
    // // articles
    ...
    dataShape.subCategories.byId[item.subcat_id] = {
      ...dataShape.subCategories.byId[item.subcat_id],
      articles: dataShape.subCategories.byId[item.subcat_id].articles.concat(
        item.id
      ),
    };
  });
