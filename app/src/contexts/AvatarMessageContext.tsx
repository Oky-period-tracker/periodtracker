...
export const AvatarMessageProvider = ({ children }: React.PropsWithChildren) => {
  ...
  }, [isAvatarFeatureLocked, translate])
  ...
  React.useEffect(() => {
    if (isAvatarFeatureLocked) {
      avatarFeatureLockMessagesRef.current = lockMessages
    }
  }, [isAvatarFeatureLocked, lockMessages])
  ...
  React.useEffect(() => {
    ...
    prevIsLockedRef.current = isAvatarFeatureLocked
  ...
  const setNextLockMessage = React.useCallback(() => {
    if (avatarFeatureLockMessagesRef.current.length === 0 || currentLockMessageIndexRef.current < 0) {
      return false
    }

    const lockMessage = avatarFeatureLockMessagesRef.current[currentLockMessageIndexRef.current]
    if (lockMessage) {
      setMessage(lockMessage.content)
      currentLockMessageIndexRef.current++

      // If we've shown all lock messages, mark as done
      if (currentLockMessageIndexRef.current >= avatarFeatureLockMessagesRef.current.length) {
        currentLockMessageIndexRef.current = -1
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      !hasShownLockMessagesRef.current
    ) {
    ...
    if (!isAvatarFeatureLocked && !message && availableMessages.length > 0) {
      const timeout = setTimeout(() => {
  ...
  React.useEffect(() => {
    ...
    if (
      (hasShownLockMessagesRef.current || !isAvatarFeatureLocked) &&
