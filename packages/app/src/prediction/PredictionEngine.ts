...
export class PredictionEngine {
  private constructor(
    ...
    private listener: Subscriber = () => null
  ) {
  ...
  public predictDay(
    ...
    if (diffDays >= cycleLength && diffDays > 0) {
      ...
      cycleStart = moment(cycleStart)
        ...
        .add(
          cycleLength +
            numberOfCyclesAheadOfCurrent *
              this.state.smartPrediction.smaCycleLength,
          "days"
        );
    ...
    if (diffDays < 0) {
      // History Predictions
      const relevantCycleHistoryEntry =
        this._getClosetCycleHistoryInfo(inputDay).closetCycle;
      ...
      const diffFromRelevantStart = inputDay.diff(
        relevantCycleHistoryEntry.cycleStartDate,
        "days"
      );
    ...
    const fertileDayStart =
      midCycle - Math.floor(this._fertileLength(cycleLength) / 2);
    const fertileDayEnd =
      midCycle + Math.floor(this._fertileLength(cycleLength) / 2);
        ...
        if (!hasFuturePredictionActive) {
          if (!_.isEmpty(this.state.actualCurrentStartDate)) {
            ...
            isFutureDate = moment(newEntry).isAfter(
              moment(checkStartDate).add(
                this.state.actualCurrentStartDate.periodLength,
                "days"
              )
            );
          }
  ...
  public userInputDispatch({
    ...
    switch (type) {
      case "adjust-mens-end":
        this._adjustMenstruatingHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
      ...
      case "current-start-adjust":
        this._adjustCurrentStartDateHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
      ...
      case "history-start-adjust":
        this._adjustHistoryStartDateHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
      ...
      case "adjust-mens-end-history":
        this._adjustMenstruatingHistoryHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
      ...
      case "future-start-adjust":
        this._adjustFutureStartDateHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
      ...
      case "adjust-future-mens-end":
        this._adjustFutureMenstruatingHandler(
          inputDay,
          errorCallBack,
          getPredictedCycles
        );
  ...
  private _currentDayChecking() {
    ...
    if (diffDays >= this.state.currentCycle.cycleLength) {
      ...
      const daysAheadOfCurrentCycleEnd =
        diffDays - this.state.currentCycle.cycleLength;
      if (
        ...
        const dayToCheck = startingPoint
          ...
          .add(
            this.state.currentCycle.cycleLength +
              i * this.state.smartPrediction.smaCycleLength,
            "days"
          );
        if (
          (!this.state.futurePredictionStatus &&
            moment(dayToCheck).isSameOrBefore(
              moment(tempCurrentCycle.cycleStart),
              "day"
            )) ||
          this.state.futurePredictionStatus
        ) {
  ...
  private _adjustMenstruatingHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
    const daysFromStart = inputDay.diff(
      this.state.currentCycle.startDate,
      "days"
    );
    ...
    if (daysFromStart < 0 && this.state.currentCycle.periodLength === 1) {
      if (this.state.history.length) {
        const newCurrentCycle = {
          ...
          cycleLength:
            this.state.history[0].cycleLength +
            this.state.currentCycle.cycleLength,
    ...
    if (daysFromStart < this.state.currentCycle.periodLength / 2) {
      ...
      if (daysFromStart <= 0) {
        const daysFromCurrentCycle = this.state.currentCycle.startDate.diff(
          tempDate,
          "days"
        );
        ...
        if (
          tempDate.clone().add(1, "days").isAfter(moment()) &&
          remainingPeriodLenghth > 0
        ) {
          ...
          const updateNewCurrentCycle = {
            startDate: this.state.history[0].cycleStartDate,
            cycleLength:
              tempDate.diff(this.state.history[0].cycleStartDate, "days") + 1,
            periodLength: this.state.history[0].periodLength,
          };
        ...
        if (
          moment(inputDay).isSame(moment().format("YYYY-MM-DD")) &&
  ...
  private _startedEarlyHandler(inputDay, errorCallBack, getPredictedCycles) {
    ...
    if (
      daysFromStart > 0 &&
      daysFromStart < this.state.currentCycle.cycleLength
    ) {
    ...
    if (daysFromStart < 0) {
      this._adjustCurrentStartDateHandler(
        ...
        errorCallBack,
        getPredictedCycles
      ); // in the event the user shifted the current cycle forward
    }
  }
  ...
  private _addNewCycleInHistory(inputDay, errorCallBack, getPredictedCycles) {
    ...
    if (_.isEmpty(relevantCycleHistoryEntry.closetCycle)) {
      ...
      if (tempHistoryArr.length) {
        if (
          inputDay.diff(
            tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate,
            "days"
          ) >= -4
        ) {
          const updateLastHistoryCycle = {
            ...
            cycleLength:
              tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate.diff(
                inputDay,
                "days"
        ...
        const addNewHisCycle = {
          ...
          cycleEndDate: tempHistoryArr[tempHistoryArr.length - 1].cycleStartDate
            .clone()
            .subtract(1, "days"),
          cycleLength: tempHistoryArr[
            tempHistoryArr.length - 1
          ].cycleStartDate.diff(inputDay, "days"),
          periodLength: 1,
        };
      ...
      if (
        moment(this.state.currentCycle.startDate).isSame(
        ...
        if (this.state.currentCycle.periodLength >= 1) {
          ...
          }
          {
            const addHistoryCycle = {
              cycleStartDate: inputDay,
              cycleEndDate: this.state.currentCycle.startDate
                .clone()
                .subtract(1, "days"),
              cycleLength: this.state.currentCycle.startDate.diff(
                inputDay,
                "days"
              ),
              periodLength: 1,
        ...
        }
        {
          ...
          if (this.state.currentCycle.startDate.diff(inputDay, "days") > 21) {
            currentCycleLength =
              this.state.currentCycle.startDate.diff(inputDay, "days") + 7;
          }
          const addNewCurrentCycle = {
            startDate: inputDay,
            periodLength: 1,
    ...
    if (moment(inputDay).isAfter(this.state.history[0].cycleStartDate)) {
      ...
      const daysFromCurrentCycle = inputDay.diff(
        this.state.currentCycle.startDate,
        "days"
      );
      if (
        ...
        const updateLastHistoryCycle = {
          ...
          cycleEndDate: inputDay.clone().subtract(1, "days"),
          cycleLength: inputDay.diff(
            this.state.history[0].cycleStartDate,
            "days"
          ),
          periodLength: this.state.history[0].periodLength,
        };
        const updateCurrentCycle = {
          ...
          startDate: inputDay,
          cycleLength:
            this.state.currentCycle.cycleLength + -daysFromCurrentCycle,
          periodLength:
            this.state.currentCycle.periodLength + -daysFromCurrentCycle,
        };
      ...
      if (daysFromInputToCloset - this.state.history[0].periodLength < 2) {
        ...
        if (inputDay.diff(this.state.currentCycle.startDate, "days") >= 0) {
          const updateCurrentCycle1 = {
            ...
            periodLength:
              inputDay.diff(this.state.history[0].cycleStartDate, "days") + 1,
          };
          const updateCurrentCycle = {
            ...this.state.currentCycle,
            startDate: this.state.history[0].cycleStartDate,
    ...
    const periodLengthOfClosestCycle =
      this.state.history[relevantCycleHistoryEntry.closestIndex].periodLength;

    // check conditions to add new cylce between 2 history cyles
    const daysFromNextCycle = inputDay.diff(
      this.state.history[relevantCycleHistoryEntry.closestIndex - 1]
        .cycleStartDate,
      "days"
    );
    const nextHistoryCycle =
      this.state.history[relevantCycleHistoryEntry.closestIndex - 1];
    const periodLengthOfNextCycle =
      this.state.history[relevantCycleHistoryEntry.closestIndex - 1]
        .periodLength;
    ...
    if (daysFromInputToCloset > 0) {
      ...
      if (
        daysFromNextCycle >= -4 &&
        daysFromInputToCloset - periodLengthOfClosestCycle >= 2
      ) {
        const tempHistoryArr = [...this.state.history];
        ...
        const updatePreviousCycle = {
          ...
          cycleEndDate: inputDay.clone().subtract(1, "days"),
          cycleLength: inputDay.diff(
            relevantCycleHistoryEntry.closetCycle.cycleStartDate,
            "days"
          ),
        };
        tempHistoryArr[relevantCycleHistoryEntry.closestIndex - 1] =
          updateNextHistoryCycle;
        tempHistoryArr[relevantCycleHistoryEntry.closestIndex] =
          updatePreviousCycle;
        this.state.history = [...tempHistoryArr];
        getPredictedCycles(true);
        return;
      ...
      if (daysFromInputToCloset - periodLengthOfClosestCycle >= 2) {
        ...
        const newHistoryEntry = {
          ...
          cycleLength: Math.abs(
            inputDay.diff(
              relevantCycleHistoryEntry.closetCycle.cycleEndDate,
              "days"
            )
          ),
          periodLength: 1,
  ...
  private _adjustCurrentStartDateHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
    ...
    if (daysFromStart === 0) {
      ...
      this.state.currentCycle.cycleLength =
        this.state.history[0].cycleLength + 1;
      this.state.currentCycle.periodLength = this.state.history[0].periodLength;
      this.state.history.shift();
      getPredictedCycles(true);
      return;
    }
    const daysFromPreviousStart = inputDay.diff(
      this.state.history[0].cycleStartDate,
      "days"
    );
    ...
    if (daysFromStart < 0 && daysFromPreviousStart > 0) {
      const relevantCycleHistoryEntry =
        this._getClosetCycleHistoryInfo(inputDay);
      if (
        inputDay.diff(this.state.history[0].cycleStartDate, "days") -
        ...
        this.state.history[relevantCycleHistoryEntry.closestIndex] =
          updateHistory;
      }
    }
    ...
    const diffToCurrentStart = todayInfo.cycleStart.diff(
      this.state.currentCycle.startDate,
      "days"
    );
  ...
  private _adjustFutureStartDateHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
  ...
  private _adjustFutureMenstruatingHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
  ...
  private async _cycleCompletion(completionDate, getPredictedCycles) {
    const adjustedCycleLength = completionDate.diff(
      this.state.currentCycle.startDate,
      "days"
    );
    this.state.history.unshift({
      cycleStartDate: this.state.currentCycle.startDate,
      cycleEndDate: moment(completionDate.clone().subtract(1, "days")).startOf(
        "day"
      ),
      periodLength: this.state.currentCycle.periodLength,
      cycleLength: adjustedCycleLength,
    });
    this.state.currentCycle.startDate = moment(
      completionDate.valueOf()
    ).startOf("day");
    this.state.smartPrediction.circularPeriodLength.push(
      this.state.currentCycle.periodLength
    );
    ...
    this.state.currentCycle.periodLength =
      this.state.smartPrediction.smaPeriodLength;
  ...
  private _adjustHistoryStartDateHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
    const { closetCycle, closestIndex } =
      this._getClosetCycleHistoryInfo(inputDay);
    ...
    if (_.isEmpty(closetCycle)) {
      ...
      const periodLengthToBe =
        this.state.history[this.state.history.length - 1].periodLength +
        -daysFromStart;
      const cycleLengthToBe =
        this.state.history[this.state.history.length - 1].cycleLength +
        -daysFromStart;
    ...
    if (cycleDay >= Math.round(cycleLength / 2)) {
      const currentCycleLengthToBe = inputDay.diff(
        closetCycle.cycleStartDate,
        "days"
      );
      ...
      if (morePresentHistoryCycle && morePresentHistoryCycle.cycleStartDate) {
        if (
          inputDay.diff(morePresentHistoryCycle.cycleStartDate, "days") > -5
        ) {
          if (currentCycleLengthToBe - closetCycle.periodLength >= 7) {
            const updateHistoryNextCycle = {
              ...morePresentHistoryCycle,
              cycleStartDate: inputDay,
              cycleLength: (morePresentHistoryCycle.cycleLength +=
                -daysFromStart),
              periodLength: (morePresentHistoryCycle.periodLength +=
                -daysFromStart),
            };
          ...
          {
            const daysFromRelevantStart = inputDay.diff(
              closetCycle.cycleStartDate,
              "days"
            );
            this.state.history[closestIndex].periodLength +=
              -daysFromRelevantStart;
            getPredictedCycles(true);
            return;
          }
        }
      ...
      if (
        currentCycleLengthToBe <
        closetCycle.periodLength + minBufferBetweenCycles
      ) {
        errorCallBack("too_close");
        return;
      }
    ...
    if (cycleDay < closetCycle.periodLength / 2) {
      ...
      if (
        this.state.history[closestIndex].periodLength +
          Math.abs(daysFromRelevantStart) <
        minPeriodLength
      ) {
      ...
      if (morePastHistoryCycle) {
        this.state.history[indexMorePast].cycleEndDate = inputDay
          .clone()
          .subtract(1, "days");
  ...
  private _adjustMenstruatingHistoryHandler(
    inputDay,
    errorCallBack,
    getPredictedCycles
  ) {
    ...
    if (
      daysFromStart < 0 &&
      !_.isEmpty(relevantCycleHistoryEntry.closetCycle)
    ) {
      const lengthToBe =
        inputDay.diff(
          relevantCycleHistoryEntry.closetCycle.cycleStartDate,
