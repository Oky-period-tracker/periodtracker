...
function* onFetchContentRequest(
  ...
  try {
    const { articles, categories, subCategories, videos } =
      yield fetchEncyclopedia();
    ...
    if (!aboutContent) {
      const localeInit = (yield select(
        selectors.currentLocaleSelector
      )) as Locale;
