...
const EYE_COLORS = ['#111111', '#945A1C', '#A88044', '#909249', '#6F979E', '#ABADB3']
 ...
 * Edit avatar screen
 */
const EditAvatarScreen: ScreenComponent<'EditAvatar'> = ({ navigation }) => {
    ...
    return (
      <View style={styles.categoryContainer}>
        {categories.map((category) => {
          ...
          const categoryIcon = getAsset(`avatar.categories.${category.key}`)
          return (
            <TouchableOpacity
              key={category.key}
    ...
    <View style={styles.optionsContainer}>
      <View style={styles.optionSection}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-left"
                size={16}
                color={skinColorPage === 0 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() =>
                setSkinColorPage(
                  Math.min(Math.ceil(SKIN_COLORS.length / COLORS_PER_PAGE) - 1, skinColorPage + 1),
                )
              }
              disabled={skinColorPage >= Math.ceil(SKIN_COLORS.length / COLORS_PER_PAGE) - 1}
              style={[
                styles.arrowButton,
                skinColorPage >= Math.ceil(SKIN_COLORS.length / COLORS_PER_PAGE) - 1 &&
        ...
        <View style={styles.colorSwatches}>
          {SKIN_COLORS.slice(
            skinColorPage * COLORS_PER_PAGE,
            (skinColorPage + 1) * COLORS_PER_PAGE,
          ).map((color, index) => {
        ...
        <View style={{ flexDirection: 'row' }}>
          <View style={styles.bodyTypeContainer}>
            {(['small', 'medium', 'large'] as const).map((bodyType) => {
              const bodyAsset = getAsset(`avatar.body.${bodyType}`)
  ...
  const renderHairOptions = () => (
    <View style={styles.optionsContainer}>
      <View style={styles.optionSection}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-left"
                size={16}
                color={hairColorPage === 0 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() =>
                setHairColorPage(
                  Math.min(Math.ceil(HAIR_COLORS.length / COLORS_PER_PAGE) - 1, hairColorPage + 1),
                )
              }
              disabled={hairColorPage >= Math.ceil(HAIR_COLORS.length / COLORS_PER_PAGE) - 1}
              style={[
                styles.arrowButton,
                hairColorPage >= Math.ceil(HAIR_COLORS.length / COLORS_PER_PAGE) - 1 &&
                  styles.arrowButtonDisabled,
              ]}
            >
              <FontAwesome
        ...
        <View style={styles.colorSwatches}>
          {HAIR_COLORS.slice(
            hairColorPage * COLORS_PER_PAGE,
            (hairColorPage + 1) * COLORS_PER_PAGE,
          ).map((color, index) => {
      ...
      <View style={[styles.optionSection, styles.lastOptionSection]}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-left"
                size={16}
                color={hairPage === 0 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() =>
                setHairPage(Math.min(Math.ceil(hairOptions.length / 3) - 1, hairPage + 1))
              }
              disabled={hairPage >= Math.ceil(hairOptions.length / 3) - 1}
              style={[
                styles.arrowButton,
                hairPage >= Math.ceil(hairOptions.length / 3) - 1 && styles.arrowButtonDisabled,
              ]}
            >
              <FontAwesome
                name="chevron-right"
                size={16}
                color={hairPage >= Math.ceil(hairOptions.length / 3) - 1 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
          </View>
        </View>
        <View style={styles.paginatedOptions}>
          {hairOptions.slice(hairPage * 3, (hairPage + 1) * 3).map((item) => {
            const hairAsset = getAsset(`avatar.hair.${item}`)
            const isSelected = avatarSelection.hairStyle === item
            return (
              <TouchableOpacity
                key={item}
                onPress={() => setAvatarSelection({ ...avatarSelection, hairStyle: item })}
                style={[styles.hairstyleOption, isSelected && styles.hairstyleOptionSelected]}
              >
                <Image
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const renderEyesOptions = () => (
    <View style={styles.optionsContainer}>
      <View style={styles.optionSection}>
        ...
        <View style={styles.colorSwatches}>
          {EYE_COLORS.slice(
            eyeColorPage * COLORS_PER_PAGE,
            (eyeColorPage + 1) * COLORS_PER_PAGE,
          ).map((color, index) => {
      ...
      <View style={[styles.optionSection, styles.lastOptionSection]}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-left"
                size={16}
                color={eyePage === 0 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() =>
                setEyePage(Math.min(Math.ceil(eyeOptions.length / 3) - 1, eyePage + 1))
              }
              disabled={eyePage >= Math.ceil(eyeOptions.length / 3) - 1}
              style={[
                styles.arrowButton,
                eyePage >= Math.ceil(eyeOptions.length / 3) - 1 && styles.arrowButtonDisabled,
              ]}
            >
              <FontAwesome
                name="chevron-right"
                size={16}
                color={eyePage >= Math.ceil(eyeOptions.length / 3) - 1 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
          </View>
        </View>
        <View style={styles.paginatedOptions}>
          {eyeOptions.slice(eyePage * 3, (eyePage + 1) * 3).map((item) => {
            const eyeAsset = getAsset(`avatar.eyes.${item}`)
  ...
  const renderClothingOptions = () => {
    ...
    const currentPageItems = clothingOptions.slice(
      ...
      <View style={styles.optionsContainer}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-left"
                size={16}
                color={clothingPage === 0 ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <FontAwesome
                name="chevron-right"
                size={16}
                color={clothingPage >= maxPage ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
          </View>
        </View>
        <View style={[styles.gridContainer, styles.lastOptionSection]}>
          {currentPageItems.map((item) => {
            const clothingImage = getAsset(`avatar.clothing.${item}`)
            ...
            return (
              ...
              >
                <Image source={clothingImage} style={styles.itemImage} resizeMode="contain" />
              </TouchableOpacity>
            )
          })}
        </View>
  ...
  const renderDeviceOptions = () => {
    const itemsPerPage = 6 // 2 rows of 3
    const currentPageItems = deviceOptions.slice(
      devicePage * itemsPerPage,
      (devicePage + 1) * itemsPerPage,
    )
    ...
    return (
      <View style={styles.optionsContainer}>
          ...
          <View style={styles.arrowButtons}>
            ...
            >
              <FontAwesome
                name="chevron-right"
                size={16}
                color={devicePage >= maxPage ? '#ccc' : '#2196F3'}
              />
            </TouchableOpacity>
          </View>
        </View>
        <View style={[styles.gridContainer, styles.lastOptionSection]}>
          {currentPageItems.map((item) => {
            const deviceImage = getAsset(`avatar.devices.${item}`)
            ...
            return (
              <TouchableOpacity
                ...
                style={[styles.itemOption, isSelected && styles.itemOptionSelected]}
              >
                <Image source={deviceImage} style={styles.itemImage} resizeMode="contain" />
              </TouchableOpacity>
            )
          })}
        </View>
    ...
    <View style={[styles.screen, { backgroundColor: '#E3F2FD' }]}>
      ...
      <View style={styles.bottomButtonsContainer}>
        ...
        <View style={styles.bottomButtons}>
          ...
          <TouchableOpacity onPress={handleSave} style={[styles.saveButton, styles.orangeButton]}>
            <Text style={styles.buttonText} enableTranslate={false}>
              Save you friend
            </Text>
          </TouchableOpacity>
...
const createStyles = (config: typeof responsiveConfig.s.avatarCustomization) =>
  StyleSheet.create({
    screen: {
      flex: 1,
      paddingHorizontal: 0,
