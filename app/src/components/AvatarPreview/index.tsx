...
import { getAvatarAsset } from '../../services/asset'
import { getDarkerSkinColor } from '../../services/colorUtils'
...
export interface AvatarPreviewProps {
  ...
  devices?: string[] | null // Support both string (old format) and array (new format)
  width?: number
  height?: number
  style?: ViewStyle
}
...
export const AvatarPreview: React.FC<AvatarPreviewProps> = ({
  ...
  const BodyComponent = React.useMemo(() => {
    if (bodyType === 'small') return BodySmall
    if (bodyType === 'large') return BodyLarge
    return BodyMedium
  }, [bodyType])
  ...
  const HairSvg = hairStyle === '00' ? null : getAvatarAsset('hair', hairStyle, bodyType)
  const SmileSvg = smile ? getAvatarAsset('smile', smile, bodyType) : null
  const EyeSvg = getAvatarAsset('eyes', eyeShape, bodyType)
  const ClothingSvg = getAvatarAsset('clothing', clothing, bodyType)
