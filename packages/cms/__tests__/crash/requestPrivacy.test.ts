...
jest.mock('../../src/services/crashAnalysisService', () => ({
  crashAnalysisService: {
    ...
    getTimeoutThreshold: () => 0,
  },
}))

describe('request URL privacy', () => {
  afterEach(() => jest.clearAllMocks())
  it.each(['ok', 'error', 'timeout'])(
    'omits query values on %s paths and diagnostics',
    async (scenario) => {
      const app = express()
      ...
      for (const mock of [
        ...
        logger.warn,
        logger.error,
        crashAnalysisService.recordTimeout,
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Incoming request',
        expect.objectContaining({ url: '/private' }),
      )
    },
  )
})
