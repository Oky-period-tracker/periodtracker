...
const s = (state: SecureReduxState) => state.answer

export const surveyHasAnswerSelector = (state: SecureReduxState, id: string) => {
  if (!s(state)[state.auth.user.id]) return false
  return id in s(state)[state.auth.user.id].surveys
}

// export const surveysWithoutAnswersSelector = (state: SecureReduxState) => {
//   return allSurveysSelectors(state).filter(({ id }) => !surveyHasAnswerSelector(state, id))
// }

export const quizHasAnswerSelector = (state: SecureReduxState, id: string) => {
  if (!s(state)[state.auth.user.id]) return false
  return id in s(state)[state.auth.user.id].quizzes
}

// Had a type error here had to add any to avoid
export const quizAnswerByDate: any = (state: SecureReduxState, date: Moment) => {
...
// Had a type error here had to add any to avoid
export const surveyAnswerByDate: any = (state: SecureReduxState, date: Moment) => {
  if (!s(state)[state.auth.user.id]) return null
...
export const quizzesWithoutAnswersSelector = (state: SecureReduxState) => {
  return allQuizzesSelectors(state).filter(({ id }) => !quizHasAnswerSelector(state, id))
}

export const cardAnswerSelector = (state: SecureReduxState, date: Moment) => {
...
export const verifyPeriodDaySelectorWithDate = (state: SecureReduxState, date: Moment) => {
  if (!state.auth.user) return {} // for the use case on info screen where there is no authed user
  if (!s(state)[state.auth.user.id]) return {}
...
export const allCardAnswersSelector = (state: SecureReduxState) => {
  if (!state.auth.user) return {} // for the use case on info screen where there is no authed user
  if (!s(state)[state.auth.user.id]) return {}
  return s(state)[state.auth.user.id]?.verifiedDates || {}
}

export const notesAnswerSelector = (state: SecureReduxState, date: Moment) => {
  if (!s(state)[state.auth.user.id]) return {}
  return s(state)[state.auth.user.id].notes[toShortISO(date)] || {}
}

export const mostAnsweredSelector = (
  state: SecureReduxState,
  startDate: Moment,
  endDate: Moment,
) => {
  if (!s(state)[state.auth.user.id]) return {}
